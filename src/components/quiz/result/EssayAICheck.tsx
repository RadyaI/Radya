...
export default function EssayAICheck({ question, userAnswer, answerKey }: EssayAICheckProps) {
  ...
  const handleCheck = async () => {
    ...
    try {
      const prompt = `
        Context: Koreksi jawaban essay singkat dari mahasiswa IT.
        Soal: "${question}"
        Jawaban User: "${userAnswer}"
        Kunci Jawaban/Poin Penting: "${answerKey}"
        Instruksi:
        1. Bandingkan jawaban user dengan kunci jawaban.
        2. Jika BENAR: Berikan apresiasi singkat.
        3. Jika SALAH/KURANG: Jelaskan singkat apa yang hilang.
        4. Jawab SINGKAT & PADAT (Maksimal 2 kalimat).
      `;
