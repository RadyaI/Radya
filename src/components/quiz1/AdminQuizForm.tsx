...
import { Plus, Trash2, AlertCircle, Image as ImageIcon, Code, Clock, FileText, ListChecks, CheckCircle } from 'lucide-react'
...
export default function AdminQuizForm({ userEmail, initialData, isEditMode = false }: Props) {
  ...
  const handleSubmit = async () => {
    if (!formData.title || !formData.description) return toast.error('Isi judul & deskripsi dulu!')
    
    const isValid = questions.every(q => {
        if (!q.question) return false
        if (formData.type === 'multiple-choice') {
            return q.options.some((o: any) => o.text) && q.options.some((o: any) => o.isCorrect)
        }
        return true
    })

    if (!isValid) return toast.error('Cek lagi! Ada soal kosong atau PG yang belum ada kunci jawaban.')

    const sanitizedQuestions = questions.map(q => ({ ...q, type: formData.type }))

    const payload = { ...formData, questions: sanitizedQuestions }
      ...
      <div className="bg-zinc-900/40 backdrop-blur-xl border border-white/10 p-6 rounded-3xl relative overflow-hidden group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div>
              <label className="text-xs text-zinc-400 ml-1 mb-1 block">Quiz Mode (Type)</label>
              <div className="grid grid-cols-2 gap-2 p-1 bg-black/40 border border-white/10 rounded-xl">
                <button
                    ...
                    className={`flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${formData.type === 'multiple-choice' ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                    <ListChecks className="w-4 h-4" /> Multiple Choice
                </button>
                <button
                    onClick={() => handleTypeChange('essay')}
                    className={`flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${formData.type === 'essay' ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                    <FileText className="w-4 h-4" /> Full Essay
                </button>
              </div>
              <p className="text-[10px] text-zinc-500 mt-1.5 ml-1">
                *Changing mode will reset/convert all existing questions.
              </p>
            </div>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                    <span className="text-sm font-mono text-blue-500 bg-blue-500/10 px-2 py-1 rounded">Q{qIdx + 1}</span>
                    {/* Badge Mode */}
                    <span className="text-xs text-zinc-500 flex items-center gap-1 bg-black/30 px-2 py-1 rounded border border-white/5">
                        {formData.type === 'essay' ? <FileText className="w-3 h-3" /> : <ListChecks className="w-3 h-3" />}
                        {formData.type === 'essay' ? 'Essay' : 'Multiple Choice'}
                    </span>
                </div>
              ...
              {formData.type === 'multiple-choice' ? (
                ...
                <div className="mb-4 p-4 bg-blue-500/5 border border-blue-500/20 rounded-xl text-sm text-blue-400 flex items-start gap-3">
                    <FileText className="w-5 h-5 flex-shrink-0" />
                    <div>
                        <p className="font-bold mb-1">Essay Mode Active</p>
                        <p className="text-blue-400/70">Users will be provided with a text area to type their answer. Grading is manual.</p>
                    </div>
                </div>
              )}
              ...
              <input 
                ...
                placeholder={formData.type === 'multiple-choice' ? "Explanation (Why is this answer correct?)" : "Grading Key (What keywords/points to check?)"}
